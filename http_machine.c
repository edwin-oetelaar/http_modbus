...
int http_machine_step(http_state_t *self, int hangup) {
    /* self, buffer length (recv_buffer), hangup flag (when peer closed==1 network error==2) */
    ...
    if (buflen > 0) {
        ...
        while (cnt < buflen) {
            /* get the byte */
            uint8_t c = self->recv_buffer[cnt];
            ...
            switch (self->current_state) {
                ...
                case 4: /* we look for the LINEFEED at the end of the first line */
                    if (c == '\n') {
                        ...
                        ns = 1000; /* unexpected char, is protol error */
                    }
                    break;
                ...
                case 5:
                    ...
                    if (!(self->line_buffer_index < sizeof self->line_buffer)) {
                        ...
                        self->line_buffer[self->line_buffer_index] = 0; /* terminate string */
                        if (self->line_buffer_index == 0) {
                            /* \r without data in the line, means end of header lines */
                            cnt++; /* char consumed */
                            ns = 7; /* read trailing \n before the body */
                        } else {
                ...
                case 7:
                    /* read LF after last header line */
                    if (c == '\n') {
                        /* cnt++; */
                        ns = 20;
                    } else {
        ...
        /* buffer is zero length handle that too */
    ...
    if (hangup)
        return -1; /* other side broke connection, so we break connection */
    else
        return 0; /* all ok */
