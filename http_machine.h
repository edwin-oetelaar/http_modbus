...
#include "ringbuffer.h"
    ...
    typedef struct {
        ...
        int VERB_code; /* code for verb, GET=1 POST=2 etc. */
        ...
        int content_length; /* from the header, needed for POST handling */
        int content_body_index; /* offset into buffer counter */
        char BODY[1024]; /* body van de POST message komt hier */
    } http_state_t;


    int http_m_init(http_state_t *self, int fd); /* constructor */

    int http_m_reset(http_state_t *self); /* reset internal state */

    int http_m_deinit(http_state_t *self); /* destructor */

    int http_m_step(http_state_t *self, int hangup); /* step though recv buffer */

   // static int http_m_step_single_byte(http_state_t *self, uint8_t c, int control_flag); /* internal non api parse input chars */
